...
% Monolithic: self-contained application, app + all extensions written in the same language, executed on the same platform
% Distributed: business logic implemented in the proxy app, extensions such as encoding done by remote units that are platform and programming language agnostic
\paragraph{Complexity}
% Monolithic: business-logic is hard to debug due to complex nested structures, extensions are simple to debug
% Distributed: business-logic is simple to debug (pipes.rip), extensions are harder to debug due to distributed setup (connection losses, remote machines, remote debuggers?)
\paragraph{Maturity}
% Monolithic: pipes were tested before and improved, new concepts are untested but at least the design is complete and consistent
% Distributed: some aspects were simplified (pipes removed), however some core and non-trivial aspects are not fully specified

While its underlying ideas potentially hold a lot of promise, they also introduce new problems:
\begin{itemize}
    \item \textbf{De-centralization:} The de-centralized approach to processing messages greatly increases the application's complexity on multiple levels.
\end{itemize}